...
| `E2E_ADMIN_PAT` | **Required in CI** (locally falls back to `gh auth token`, or set `E2E_ADMIN_PAT_FILE`). A `muratmaga` (a `morphocloud-admins` member) **fine-grained** PAT, resource owner `MorphoCloud`, scoped to **`Test-Instances` + `MorphoCloudWorkflow`**, permissions **Contents + Workflows + Actions + Issues: read/write** (+ Metadata: read). *Workflows*-write is required because Stage 0 pushes `.github/workflows/` changes to Test-Instances; *Actions* drives `workflow_dispatch`; *Issues* drives `/approve` + labels. Local file: `~/.ssh/GH-morphocloud-e2e-admin`. **⏰ Expires `2026-09-07` (90-day rotation).** When it lapses, every provisioning suite fails at Stage 0 with a `401`/`403` — regenerate a fine-grained PAT with the same scopes, update the `E2E_ADMIN_PAT` secret + the local file, and verify with the `cheap` suite. Expiry is monitored by `MorphoCloudAnalytics/.github/workflows/token-expiry-check.yml` (emails `ADMIN_EMAIL` ~28 days out). |
